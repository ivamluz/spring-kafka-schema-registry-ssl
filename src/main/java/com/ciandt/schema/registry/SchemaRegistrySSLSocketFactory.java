...
public class SchemaRegistrySSLSocketFactory {
	...
	public SSLSocketFactory getSslSocketFactory() throws SSLException {
		try {
			...
			final String protocol = Optional.ofNullable(properties.getSsl().getProtocol()).orElseThrow(() -> new IllegalArgumentException("ssl.protocol not found"));
	...
	private KeyManager[] getKeyManagers() throws NoSuchAlgorithmException, KeyStoreException, CertificateException, IOException, UnrecoverableKeyException {
		
		final String keyManagerAlgorithm = StringUtils.isEmpty(properties.getSsl().getKeyManagerAlgorithm()) ? KeyManagerFactory.getDefaultAlgorithm() : properties.getSsl().getKeyManagerAlgorithm();
	...
	private TrustManager[] getTrustManagers() throws NoSuchAlgorithmException, KeyStoreException, CertificateException, IOException {
		...
		final String trustManagerAlgorithm = StringUtils.isEmpty(properties.getSsl().getTrustManagerAlgorithm()) ? TrustManagerFactory.getDefaultAlgorithm() : properties.getSsl().getTrustManagerAlgorithm();
	...
	private KeyStore createKeyStore() throws KeyStoreException, NoSuchAlgorithmException, CertificateException, IOException {
		...
		final Resource keyStoreLocation = Optional.ofNullable(properties.getSsl().getKeyStoreLocation()).orElseThrow(() -> new IllegalArgumentException("ssl.key-store-location not found"));
	...
	private KeyStore createTrustStore() throws KeyStoreException, NoSuchAlgorithmException, CertificateException, IOException {
		...
		final Resource trustStoreLocation = Optional.ofNullable(properties.getSsl().getTrustStoreLocation()).orElseThrow(() -> new IllegalArgumentException("ssl.trust-store-location not found"));
